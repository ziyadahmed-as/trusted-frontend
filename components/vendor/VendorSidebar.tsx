                ...
                {group.items.map((item, itemIdx) => {
                  ...
                  return (
                    <li key={itemIdx}>
                      ...
                      >
                        <item.icon className={cn("w-5 h-5", isActive ? "text-[#10b981]" : "text-[#dee4ee]")} />
                        {item.name}
                        {isActive && (
      ...
      <div className="mt-auto px-6 py-6 border-t border-[#2e3a47]">
        <div className="flex items-center gap-3 mb-6">
            ...
            <div className="overflow-hidden">
                <p className="text-sm font-medium text-white truncate">{user?.username || 'Vendor'}</p>
                <p className="text-xs text-[#8a99af] truncate lowercase">{user?.role === 'VENDOR' ? 'Store Owner' : 'Vendor'}</p>
            </div>
        </div>
        <button
          onClick={logout}
