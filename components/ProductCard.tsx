    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center text-gray-200 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-700 ease-out">
          <ShoppingBag
            className={cn("opacity-10", isFeatured ? "w-40 h-40" : "w-24 h-24")}
          />
        </div>
        ...
        <div className="absolute top-6 left-6 flex flex-col gap-2 z-10">
          {product.average_rating &&
            parseFloat(product.average_rating.toString()) >= 4.5 && (
      ...
      <div className={cn("flex flex-col flex-1", isFeatured ? "p-8" : "p-6")}>
        <div className="flex items-center gap-3 mb-4">
          ...
          {product.average_rating && (
            <div className="flex items-center gap-1 text-[10px] font-bold text-gray-400">
              <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
              <span>
                {parseFloat(product.average_rating.toString()).toFixed(1)}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="mt-auto flex items-center justify-between pt-6 border-t border-gray-50">
          ...
          <Link
            ...
            className="w-12 h-12 rounded-2xl bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-indigo-600 hover:text-white hover:rotate-12 transition-all group/btn"
          >
