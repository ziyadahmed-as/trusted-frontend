    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-6">
        <div className="max-w-md w-full bg-white p-12 rounded-[3.5rem] border border-gray-100 text-center shadow-xl shadow-gray-200/50">
          ...
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-8 py-4 bg-gray-900 text-white rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-indigo-600 transition-all active:scale-95"
          >
              ...
              <div className="p-8 bg-gray-50 rounded-[2.5rem] flex items-center justify-between group">
                ...
                {product.vendor_id && (
                  <Link
                    ...
                    className="px-6 py-3 bg-white border border-gray-200 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-gray-900 hover:text-white transition-all"
                  >
                    Visit Store
                  </Link>
                )}
        ...
        <div className="mt-40 space-y-24">
          <section className="space-y-12">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-gray-100 pb-12">
              <div className="space-y-4">
                <h2 className="text-xs font-black text-indigo-600 uppercase tracking-[0.3em] italic">
                  Voice of Community
                </h2>
                <h3 className="text-5xl font-black text-gray-900 tracking-tighter italic">
                  Customer <span className="text-gray-400">Experience</span>
                </h3>
              </div>
              <div className="flex flex-col items-start md:items-end gap-2">
                ...
                <p className="text-xs font-bold text-gray-400 italic">
                  Based on {product.total_reviews || 0} global reviews
                </p>
              </div>
            </div>
            ...
            {user ? (
              <div className="bg-gray-50 rounded-[3rem] p-10 border border-gray-100 shadow-sm space-y-8">
                ...
                <form onSubmit={handleReviewSubmit} className="space-y-6">
                  ...
                  {reviewError && (
                    <p className="text-[10px] font-black text-rose-500 uppercase tracking-widest px-4">
                      {reviewError}
                    </p>
                  )}
                </form>
              ...
              <div className="p-10 bg-indigo-50/50 rounded-[3rem] border border-indigo-100 text-center">
                <p className="text-sm font-bold text-gray-600 mb-4 italic">
                  Please sign in to share your experience with this product.
                </p>
                <Link
                  href="/login"
                  className="inline-flex px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100"
                >
                  Sign In Now
                </Link>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {product.reviews && product.reviews.length > 0 ? (
                product.reviews.map((review: any) => (
                  <div
                    key={review.id}
                    className="p-10 bg-white border border-gray-50 rounded-[3.5rem] space-y-6 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-500 group"
                  >
