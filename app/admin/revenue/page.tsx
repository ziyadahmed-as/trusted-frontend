...
import {
  ...
  ChevronRight, Loader2, AlertCircle, Wallet,
...
export default function RevenueAnalyticsPage() {
  ...
  const revenueData = stats?.growth_data || [28000, 34000, 29000, 41000, 38000, 52000, 46000, 61000, 55000, 72000, 68000, 89000];
  const maxRev = Math.max(...revenueData);

  const totalRevenue = stats?.total_revenue ?? revenueData.reduce((a: number, b: number) => a + b, 0);
  ...
  const colorMap: Record<string, { bg: string; text: string; glow: string; spark: string }> = {
    indigo: { bg: 'bg-indigo-50', text: 'text-indigo-600', glow: 'shadow-indigo-100', spark: '#6366f1' },
    emerald: { bg: 'bg-emerald-50', text: 'text-emerald-600', glow: 'shadow-emerald-100', spark: '#10b981' },
    violet: { bg: 'bg-violet-50', text: 'text-violet-600', glow: 'shadow-violet-100', spark: '#8b5cf6' },
    amber: { bg: 'bg-amber-50', text: 'text-amber-600', glow: 'shadow-amber-100', spark: '#f59e0b' },
  };
  ...
  if (loading) {
    return (
      <div className="h-[70vh] flex flex-col items-center justify-center gap-6">
        ...
        <p className="text-sm font-black text-indigo-600 uppercase tracking-[0.3em] animate-pulse">Loading Revenue Data...</p>
      </div>
    );
  }
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-bold tracking-tight max-w-xl mt-2">
            Real-time financial performance across the entire TrestBiyyo marketplace ecosystem.
          </p>
        </div>
            ...
            {['Daily', 'Weekly', 'Monthly', 'Annual'].map((r) => (
              <button
                ...
                className={cn(
                  ...
                  activeRange === r ? "bg-white text-gray-900 shadow-sm border border-gray-100" : "text-gray-400 hover:text-gray-600"
                )}
              >
                {r}
              </button>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div className={cn("w-11 h-11 rounded-2xl flex items-center justify-center shadow-lg", colors.bg, colors.glow)}>
              ...
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1 mb-3">{card.label}</p>
              <div className="flex items-center gap-2">
                ...
                <span className={cn("text-[10px] font-black", card.up ? "text-emerald-500" : "text-amber-500")}>{card.trend}</span>
                <span className="text-[10px] font-bold text-gray-300">{card.sub}</span>
              </div>
            </motion.div>
          );
        })}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-8 bg-white p-10 rounded-[3rem] border border-gray-100 shadow-[0_20px_60px_rgba(0,0,0,0.03)]">
          <div className="flex items-center justify-between mb-10">
            <div>
              <h3 className="text-2xl font-black text-gray-900 tracking-tighter">Revenue Flow</h3>
              <p className="text-gray-400 font-bold text-sm">Monthly gross revenue — {new Date().getFullYear()}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            {MONTHS.map((m) => (
              <span key={m} className="text-[9px] font-black text-gray-300 uppercase tracking-widest flex-1 text-center">{m}</span>
            ))}
          </div>
          <div className="mt-8 pt-6 border-t border-gray-50 flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Gross Revenue</p>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-gray-100 rounded-full" />
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Past Months</p>
            </div>
          </div>
        </div>
        ...
        <div className="xl:col-span-4 bg-gray-900 p-8 rounded-[3rem] shadow-2xl shadow-gray-200/50 flex flex-col">
          <div className="mb-8">
            <h3 className="text-2xl font-black text-white tracking-tighter">Financial Breakdown</h3>
            <p className="text-gray-400 text-sm font-bold mt-1">Platform commission summary</p>
          </div>
          <div className="space-y-4 flex-1">
            {[
              { label: 'Gross Volume', val: `$${totalRevenue.toLocaleString()}`, pct: 100, color: 'bg-indigo-500' },
              { label: 'Platform Fees', val: `$${platformFees.toLocaleString()}`, pct: 5, color: 'bg-emerald-500' },
              { label: 'Vendor Share', val: `$${vendorPayouts.toLocaleString()}`, pct: 88, color: 'bg-violet-500' },
              { label: 'Ops Reserve', val: `$${Math.round(totalRevenue * 0.07).toLocaleString()}`, pct: 7, color: 'bg-amber-500' },
            ].map((item, i) => (
      ...
      <section className="bg-white rounded-[3rem] border border-gray-100 shadow-[0_20px_60px_rgba(0,0,0,0.03)] overflow-hidden">
        <div className="p-8 border-b border-gray-50 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm font-bold text-gray-400 mt-1">Recent financial activity across the platform</p>
          </div>
        ...
        <div className="overflow-x-auto">
          {txLoading ? (
            ...
            <div className="py-20 text-center">
              ...
              <p className="text-sm font-black text-gray-400 uppercase tracking-widest">No transactions recorded yet</p>
            </div>
          ) : (
                  ...
                  {transactions.slice(0, 10).map((tx: any, i: number) => (
                    ...
                    >
                      <td className="px-8 py-5">
                        <p className="text-sm font-black text-gray-900 uppercase tracking-tight">TXN-{String(tx.id || i + 1).padStart(6, '0')}</p>
                        <p className="text-[11px] font-bold text-gray-400">{tx.description || 'Platform Transaction'}</p>
                      </td>
                      ...
                      <td className="px-6 py-5">
                        <p className="text-sm font-black text-gray-900">${parseFloat(tx.amount || 0).toLocaleString()}</p>
                      </td>
                      <td className="px-6 py-5">
                        <span className={cn(
                      ...
                      <td className="px-8 py-5 text-right">
                        <p className="text-[11px] font-bold text-gray-400">
                          {tx.created_at ? new Date(tx.created_at).toLocaleDateString() : 'Recently'}
                        </p>
                      </td>
                    </motion.tr>
                  ))}
