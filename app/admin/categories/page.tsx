  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await fetch(
        `${apiClient.getCurrentApiUrl()}/products/categories/`,
        {
          method: "POST",
          headers: {
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-bold tracking-tight max-w-xl mt-2">
            Organize the marketplace taxonomy. All {categories.length}{" "}
            categories, {totalProducts} products indexed.
          </p>
        </div>
                      ...
                      >
                        <td className="px-8 py-5">
                          <div className="flex items-center gap-3">
                            ...
                            >
                              <FolderOpen
                                className={cn("w-5 h-5", palette.text)}
                              />
                            </div>
