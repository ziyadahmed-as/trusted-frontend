...
export default function InventoryControlPage() {
  ...
  const inStock = products.filter(
    (p) => getStockStatus(p.stock_quantity ?? 999) === "IN_STOCK",
  ).length;
  const lowStock = products.filter(
    (p) => getStockStatus(p.stock_quantity ?? 999) === "LOW_STOCK",
  ).length;
  const outOfStock = products.filter(
    (p) => getStockStatus(p.stock_quantity ?? 999) === "OUT_OF_STOCK",
  ).length;
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 font-bold tracking-tight max-w-xl mt-2">
            Real-time visibility across {products.length} SKUs from all verified
        ...
        {[
          ...
          <motion.button
            ...
            className={cn(
              ...
              activeFilter ===
                (i === 0 ? "ALL" : Object.keys(stockConfig)[i - 1])
                        ...
                        <td className="px-8 py-5">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <p className="text-sm font-black text-gray-900 truncate max-w-[180px] group-hover:text-indigo-600 transition-colors">
                                {product.name}
                              </p>
                        ...
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-2">
                            <div className="w-7 h-7 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 text-[10px] font-black">
                              {(product.vendor_name || product.vendor || "V")
                                .toString()
                                .charAt(0)
                                .toUpperCase()}
                            </div>
                            <p className="text-xs font-bold text-gray-600 truncate max-w-[100px]">
        ...
        {filtered.length > 50 && (
          <div className="p-6 text-center border-t border-gray-50">
            <p className="text-xs font-bold text-gray-400">
              Showing first 50 of {filtered.length} products
            </p>
          </div>
        )}
      </section>
