    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="rounded-sm border border-[#e2e8f0] bg-[#10b981] py-6 px-7.5 shadow-sm relative overflow-hidden group">
          ...
          <div className="mt-4">
            <span className="text-xs font-medium text-white bg-white/20 px-3 py-1 rounded inline-block">
              Safe to Disburse
            </span>
          </div>
        </div>
        ...
        <div className="rounded-sm border border-[#e2e8f0] bg-white py-6 px-7.5 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <h4 className="text-2xl font-bold text-black mb-1">
                ${summary.pending_balance.toFixed(2)}
              </h4>
          ...
          <p className="mt-4 text-xs font-medium text-[#f59e0b] flex items-center gap-1">
            <AlertCircle className="w-3 h-3" /> Released once orders are
            Delivered
          </p>
        </div>
                  ...
                  {transactions.length === 0 ? (
                    ...
                    transactions.map((tx) => (
                      ...
                      >
                        ...
                        <td className="py-5 px-4 font-medium text-[#10b981]">
                          +${tx.vendor_payout_amount}
                        </td>
                  ...
                  {payouts.length === 0 ? (
                    ...
                    payouts.map((po) => (
                      ...
                      >
                        ...
                        <td className="py-5 px-4 text-sm text-[#64748b]">
                          {po.processed_at
                            ? new Date(po.processed_at).toLocaleDateString()
                            : "In Queue"}
                        </td>
                      </tr>
                    ))
          ...
          <div className="rounded-sm border border-[#e2e8f0] bg-white px-5 py-6 shadow-sm sm:px-7.5">
            <div className="mb-6 flex justify-between items-center">
              ...
              <button
                ...
                className="flex h-8 w-8 items-center justify-center rounded-full bg-[#f7f9fc] text-[#3c50e0] hover:bg-[#3c50e0] hover:text-white transition"
              >
            ...
            <div className="space-y-4">
              {methods.length === 0 ? (
                ...
                methods.map((m) => (
                  <div
                    key={m.id}
                    className="rounded-sm border border-[#e2e8f0] p-4 relative group"
                  >
                    {m.is_primary && (
                      <div className="absolute -top-2 -right-2 px-2 py-0.5 bg-[#10b981] text-white text-[10px] font-bold rounded shadow-sm">
                        Primary
                      </div>
                    )}
                    <div className="flex items-center gap-3 mb-2">
                      {m.method_type === "BANK" ? (
                        <Banknote className="w-5 h-5 text-[#3c50e0]" />
                      ) : (
                        <Wallet className="w-5 h-5 text-[#f59e0b]" />
                      )}
                      <h4 className="font-semibold text-black">
                        {m.bank_name || "Mobile Wallet"}
                      </h4>
                    </div>
