...
import { 
  Plus, Search, Archive, Package,
  Edit, Trash2, Eye, XCircle, Tag,
  ChevronLeft,
  Loader2
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-black md:text-3xl">Products Dashboard</h2>
          <p className="text-sm font-medium text-[#64748b]">Manage your store inventory, pricing, and visibility.</p>
        </div>
        <button 
              ...
              {products.length === 0 ? (
                <tr>
                   <td colSpan={5} className="py-10 text-center">
                     <p className="text-[#64748b] font-medium">No products found. Add your first product to get started.</p>
                   </td>
                </tr>
              ) : (
                products.map((prod) => (
                  <tr key={prod.id} className="border-b border-[#e2e8f0] last:border-b-0 hover:bg-[#f9fafb] transition">
                    <td className="py-5 px-4 pl-9 xl:pl-11">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <h5 className="font-medium text-black">{prod.name}</h5>
                    ...
                    <td className="py-5 px-4">
                      <p className="text-black">${parseFloat(prod.price).toFixed(2)}</p>
                    </td>
                    <td className="py-5 px-4">
                      <p className={cn("inline-flex rounded-full px-3 py-1 text-sm font-medium", 
                         prod.stock > 10 ? "bg-[#10b981]/10 text-[#10b981]" : 
                         prod.stock > 0 ? "bg-[#f59e0b]/10 text-[#f59e0b]" : 
                    ...
                    <td className="py-5 px-4 pr-9 xl:pr-11 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <button title="View Store" className="text-[#64748b] hover:text-[#10b981] transition">
                           <Eye className="w-5 h-5" />
                        </button>
                        {prod.vendor === user?.email && (
                          <>
                            <button title="Edit" className="text-[#64748b] hover:text-[#3c50e0] transition">
                               <Edit className="w-5 h-5" />
                            </button>
                            <button onClick={() => handleDelete(prod.id)} title="Delete" className="text-[#64748b] hover:text-[#d34053] transition">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-sm border border-[#e2e8f0] bg-white shadow-default">
             <div className="border-b border-[#e2e8f0] py-4 px-6.5 flex justify-between items-center">
                 ...
                 <button onClick={() => setShowModal(false)} className="text-[#64748b] hover:text-black">
                    <XCircle className="w-5 h-5" />
                 </button>
             </div>
             <form id="product-form" onSubmit={handleAddProduct} className="p-6.5">
                 <div className="mb-4.5">
                     <label className="mb-2.5 block text-black">Product Name <span className="text-[#d34053]">*</span></label>
                     <input 
                       required
                       name="name"
                       value={formData.name}
                 ...
                 <div className="mb-4.5 flex flex-col gap-4.5 sm:flex-row">
                     <div className="w-full sm:w-1/2">
                         <label className="mb-2.5 block text-black">Category <span className="text-[#d34053]">*</span></label>
                         <select 
                           required
                           name="category"
                     ...
                     <div className="w-full sm:w-1/2">
                         <label className="mb-2.5 block text-black">Price ($) <span className="text-[#d34053]">*</span></label>
                         <input 
                           required
                           type="number"
                           step="0.01"
                 ...
                 <div className="mb-4.5">
                     <label className="mb-2.5 block text-black">Initial Stock <span className="text-[#d34053]">*</span></label>
                     <input 
                       required
                       type="number"
                       min="0"
                 ...
                 <div className="mb-6">
                     <label className="mb-2.5 block text-black">Description <span className="text-[#d34053]">*</span></label>
                     <textarea 
                       required
                       name="description"
                 ...
                 >
                   {modalLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Create Product'}
                 </button>
             </form>
          </div>
        </div>
